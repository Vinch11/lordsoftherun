-- Lets the teacher attach a question to a manually-placed grid bonus: the
-- first team to reach it must answer correctly to make it explode (claim
-- the cells, exactly like a contact-triggered bonus). A wrong answer
-- consumes it instead — it just disappears, same "already claimed"
-- mechanism as a normal explosion, just with no cells painted. The answer
-- never reaches a participant: it lives in grid_bonus_answers (owner-only
-- SELECT, no client INSERT/UPDATE/DELETE policy at all) and is checked
-- server-side by answer_grid_bonus.
...
-- No INSERT/UPDATE/DELETE policy: only add_grid_bonus_with_question()
-- (SECURITY DEFINER, below) ever writes here, so the answer can never
-- reach a participant no matter what the client sends.

-- Owner-checked insert of a bonus + its secret answer in one transaction,
-- so a question bonus can never exist without its answer or vice versa.
...
-- Checks the answer server-side and atomically resolves the bonus: the
-- first submitted answer wins the race (right or wrong) — a later
-- submission on the same bonus simply gets false back, "already resolved".
